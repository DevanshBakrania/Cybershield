...
// ✨ NEW: Imports for Deep SIM Diagnostics
...
class DeviceService {
  ...
  Future<DeviceAudit> getFullAudit() async {
    ...
    // ─────────────────────────
    // CARRIER (Deep SIM Diagnostics)
    // ─────────────────────────
    try {
      final result = await Connectivity().checkConnectivity();

      if (result.contains(ConnectivityResult.mobile)) {
        carrier = "Mobile Network"; // Fallback name

        // ✨ NEW: Request phone state permissions to read the physical SIM card
        ...
        if (status.isGranted) {
          if (Platform.isAndroid) {
            ...
            if (androidInfo != null && androidInfo.telephonyInfo != null && androidInfo.telephonyInfo!.isNotEmpty) {
              // Grabs the literal name off the SIM card
              carrier = androidInfo.telephonyInfo!.first.carrierName ?? "Mobile Network";
            }
          } else if (Platform.isIOS) {
            final iosInfo = await CarrierInfo.getIosInfo();
            if (iosInfo != null && iosInfo.carrierData != null && iosInfo.carrierData!.isNotEmpty) {
              carrier = iosInfo.carrierData!.first.carrierName ?? "Mobile Network";
            }
          }
        } else {
          carrier = "Mobile Data (Perms Denied)";
        }
